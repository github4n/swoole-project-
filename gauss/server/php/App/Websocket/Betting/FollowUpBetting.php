...
class FollowUpBetting extends CheckLogin
{
    public function onReceiveLogined(Context $context, Config $config)
    {
        ...
        //追号内容
        if (empty($chase_mode)) {
            $context->reply(['status'=>406,'msg'=>'请选号']);
            return;
        }
        ...
        if ($stop_mode >1 || $stop_mode<0) {
            $context->reply(['status'=>407,'msg'=>'请选号']);
            return;
        }
        //判断该层级的用户余额是否被冻结
        ...
        if(!empty($auth)){
            if(in_array("bet_stop",json_decode($auth)) || in_array("balance_freeze",json_decode($auth))){
                $context->reply(["status"=>230,"msg"=>"该账户禁止投注,请联系客服"]);
                return;
            }
        }
        ...
        if($acceptable == 0){
            $context->reply(["status"=>400,"msg"=>"该彩票暂停销售"]);
            return;
        }
        //投注内容详细参数判断并计算金额基数
        $ruleArray = [];
        ...
        foreach ($rule_list as $rule) {
            ...
            if (!isset($rule['rebate_rate']) || $rule['rebate_rate'] > $rebate_max) {
                $context->reply(['status'=>410,'msg'=>'返点金额有误,请选择正确的返点']);
                return;
            }
            //检测该层级用户是否是禁止返点
            if(!empty($authArray)){
        ...
        $sumMoney  = 0;
            foreach ($period_list as $key=>$value) {
                if (!isset($value['period']) || empty($value['period'])) {
                    $context->reply(['status'=>413,'msg'=>'请选号']);
                    return;
                }
                $period_param = [
                    ":game_key" => $game_key,
                ...
                $res = [];
                foreach ($lottery_mysql->``($check_period_sql,$period_param) as $dat) {
                    $res = $dat;
                }
                if (empty($res)) {
                    $context->reply(['status'=>430,'msg'=>'请选号']);
                    return;
                }
                ...
                if (!isset($value['multiple']) || !is_numeric($value['multiple']) || $value['multiple'] < 0) {
                    $context->reply(['status'=>414,'msg'=>'请输入正确的投注的追投的倍数']);
                    return;
                }
        ...
        if ($sumMoney > intval($balance)) {
            $context->reply(['status'=>418,'msg'=>'余额不足']);
            return;
        }
