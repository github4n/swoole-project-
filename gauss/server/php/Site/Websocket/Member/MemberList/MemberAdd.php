 ...
 * MemberAdd class.
 *
 * @description   会员管理-会员列表-新增会员
 * @Author  blake
 * @date  2019-05-08
 * @links  Member/MemberList/MemberAdd {"user_key":"user123","user_password":"user123"}
 * @modifyAuthor   blake
...
class MemberAdd extends CheckLogin
{
    public function onReceiveLogined(Context $context, Config $config)
    {
        ...
        // 验证规则
        if (empty($user_key)) {
            $context->reply(['status' => 207, 'msg' => '请输入6-12位英文或者数字的用户名']);

            return;
        }
        $preg = '/^[0-9A-Za-z]{6,12}$/';
        if (!preg_match($preg, $user_key)) {
            $context->reply(['status' => 204, 'msg' => '请输入6-12位英文或者数字的用户名']);

            return;
        }
        if (empty($user_password)) {
            $context->reply(['status' => 205, 'msg' => '请输入6-20位英文数字组合']);
        ...
        $params = [':user_id' => $user_id, ':user_key' => strtolower($user_key), ':password_hash' => $user_password];
        try {
            $mysql->execute($sql, $params);
            //记录日志
            $sql = 'INSERT INTO operate_log SET staff_id=:staff_id, operate_key=:operate_key, detail=:detail,client_ip= :client_ip';
