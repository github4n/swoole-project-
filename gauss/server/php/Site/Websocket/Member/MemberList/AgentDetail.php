 ...
 * AgentDetail class.
 *
 * @description   会员管理-会员列表-代理线
 * @Author  blake
 * @date  2019-05-08
 * @links   Member/MemberList/AgentDetail {"user_id":"2","type":1}
...
class AgentDetail extends CheckLogin
{
    public function onReceiveLogined(Context $context, Config $config)
    {
        ...
        if (!empty($MasterId)) {
            ...
            foreach ($staff_mysql->query($sql, [':staff_id' => $staffId]) as $rows) {
            ...
            if (!empty($user_layer)) {
                foreach ($user_layer as $item) {
                    ...
                    $users = [];
                    foreach ($user_mysql->query($sql, [':layer_id' => $item]) as $row) {
                        $users = $row;
                    }
            ...
            foreach ($list as $key => $val) {
                ...
                if ($ip != '') {
                    ...
                    if (!empty($ipSaved)) {
                        ...
                        $ip_result = iterator_to_array($mysqlPublic->query($ip_sql, [':ip_net' => $ipTranslation]));
                        if (!empty($ip_result)) {
