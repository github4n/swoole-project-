    ...
    public function onReceiveLogined(Context $context, Config $config)
    {
        ...
        if (empty($auth)) {
            ...
            foreach ($staff_mysql->query($sql, [':route_id' => $moneyInfo['route_id']]) as $row) {
                $info['coupon_max'] = empty($row['coupon_max']) ? 0 : $row['coupon_max'];
                $info['coupon_rate'] = empty($row['coupon_rate']) ? 0 : $row['coupon_rate'];
                $info['coupon_times'] = empty($row['coupon_times']) ? 0 : $row['coupon_times'];
                $info['coupon_audit_rate'] = empty($row['coupon_audit_rate']) ? 0 : $row['coupon_audit_rate'];
            }
            ...
            $money_info = [];
            foreach ($mysql->query($sql, [':user_id' => $user_id, ':start' => $start, ':end' => $end]) as $rows) {
                $money_info = $rows;
            }
        ...
        $finish_money = isset($moneyInfo['launch_money']) ? $moneyInfo['launch_money'] : 0;
        if ($MasterId != 0) {
