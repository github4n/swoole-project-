 ...
 *
 *
 * @description  现金系统-出款审核-未审核
 * @Author  Rose
 * @date  2019-05-07
 * @links  Cash/WithdrawReview/UnreviewedList
 * @param  status 2正在审核 1等待审核
 * @modifyAuthor   Rose
...
class UnreviewedList extends CheckLogin
{
    public function onReceiveLogined(Context $context, Config $config)
    {
        ...
        if ($masterId == 0) {
            $sql = 'SELECT user_id,withdraw_serial,user_key,layer_id,bank_name,launch_money,bank_branch,withdraw_money,handling_fee,lock_type,deposit_audit,account_number,account_name,'.' launch_time,accept_staff_id,launch_device,finish_time,cancel_time,accept_time,reject_time'.' FROM withdraw_intact WHERE  ((must_inspect=1 AND accept_time is null and reject_time is null) or (lock_type = 1 and (accept_time is null or reject_time is null)))';
            $total_sql = 'SELECT withdraw_serial FROM withdraw_intact WHERE ((must_inspect=1 AND accept_time is null and reject_time is null) or (lock_type = 1 and (accept_time is null or reject_time is null))) ';
            $params = [];
        } else {
            $sql = 'SELECT user_id,withdraw_serial,user_key,layer_id,bank_name,launch_money,bank_branch,withdraw_money,handling_fee,lock_type,deposit_audit,account_number,account_name,'.' launch_time,accept_staff_id,launch_device,finish_time,cancel_time,accept_time,reject_time'.' FROM withdraw_intact WHERE ((must_inspect=1 AND accept_time is null and reject_time is null) or (lock_type = 1 and (accept_time is null or reject_time is null))) AND layer_id in :layer_list and launch_money <= :launch_money';
            $total_sql = 'SELECT withdraw_serial FROM withdraw_intact WHERE ((must_inspect=1 AND accept_time is null and reject_time is null) or (lock_type = 1 and (accept_time is null or reject_time is null))) AND layer_id in :layer_list and launch_money <= :launch_money ';
        ...
        if (!empty($status)) {
            if ($status == 1) {
                $sql .= ' AND launch_time > 0 AND accept_time is null AND reject_time is null and lock_type is null';
                $total_sql .= ' AND launch_time > 0 AND accept_time is null AND reject_time is null and lock_type is null';
            } elseif ($status == 2) {
                $sql .= ' AND launch_time > 0 AND accept_time is null AND reject_time is null and lock_type =1';
                $total_sql .= ' AND launch_time > 0 AND accept_time is null AND reject_time is null and lock_type =1';
            }
        }
        if (!empty($layer_id)) {
        ...
        foreach ($config->deal_list as $deal) {
            ...
            if (!empty($list)) {
                foreach ($list as $key => $val) {
                    $sqls = 'select user_id from withdraw_intact where user_id=:user_id and launch_time<:launch_time and finish_time>0';
                    $is_first = $mysql->execute($sqls, [':user_id' => $val['user_id'], ':launch_time' => $val['launch_time']]);
                    ...
                    $withdraw['is_first'] = $is_first > 0 ? '否' : '是';
                    ...
                    if (!empty($val['launch_time']) && empty($val['finish_time']) && empty($val['cancel_time']) && empty($val['accept_time']) && $val['lock_type'] == 1) {
