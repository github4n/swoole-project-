...
/*
 *
 * @description   现金系统-公司入款列表
 * @Author  Rose
 * @date  2019-04-26
 * @links  Cash/DepositAccount/DepositBankList {"acceptable":1}  1启用，2停用
 * @modifyAuthor
 * @modifyDate
 *
 * */
...
class DepositBankList extends CheckLogin
{
    public function onReceiveLogined(Context $context, Config $config)
    {
        ...
        $sql = 'SELECT * FROM deposit_passage_bank_intact';
        $total_sql = 'SELECT passage_id FROM deposit_passage_bank_intact';
        $param = [];
        $params = [];
        if (!empty($acceptable)) {
            if ($acceptable == 1) {
