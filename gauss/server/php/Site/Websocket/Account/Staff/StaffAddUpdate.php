...
class StaffAddUpdate extends CheckLogin
{
    public function onReceiveLogined(Context $context, Config $config)
    {
        ...
        $params = [
            ":staff_id" => $staff_id,
            ":deposit_limit" => $this->intercept_num($deposit_limit),
            ":withdraw_limit" => $this->intercept_num($withdraw_limit),
        ...
        //添加会员层级信息
        if (!empty($staff_level)) {
            foreach ($staff_level as $k => $v) {
                $staff_levels[$k]['layer_id'] = $v;
            }
            $mysql->staff_layer->load($staff_levels, ['staff_id' => $staff_id], '');
        }
        //记录日志
