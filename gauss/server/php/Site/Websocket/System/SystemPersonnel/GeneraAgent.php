 ...
 * @author： leo
 * @date：   2019-04-08   
 * @link：   System/SystemPersonnel/GeneraAgent {"staff_name":"","major_name":"","minor_name":""}
    ...
    public function onReceiveLogined(Context $context, Config $config)
    {
        ...
        if ($StaffGrade == 0) {
            ...
            $sql = "SELECT * FROM staff_struct_agent 
                WHERE 1=1" . $staff_name . $major_name . $shareholder . $order;
        } elseif ($StaffGrade == 1) {
            //大股东
            $sql = "SELECT * FROM staff_struct_agent 
                WHERE major_id = :major_id" . $staff_name . $major_name . $shareholder . $order;
            $param[':major_id'] = $master_id;
        } elseif ($StaffGrade == 2) {
            //股东
            $sql = "SELECT * FROM staff_struct_agent 
                WHERE minor_id = :minor_id" . $staff_name . $major_name . $shareholder . $order;
        ...
        if (!empty($list)) {
            foreach ($list as $key => $val) {
                ...
                $agent_list[$key]["bet_rate"] = floatval($info["grade3_bet_rate"]);
                $agent_list[$key]["profit_rate"] = floatval($info["grade3_profit_rate"]);
                $agent_list[$key]["fee_rate"] = floatval($info["grade3_fee_rate"]);
                $agent_list[$key]["tax_rate"] = floatval($info["grade3_tax_rate"]);
