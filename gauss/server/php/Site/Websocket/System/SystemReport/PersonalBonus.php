...
class PersonalBonus extends CheckLogin
{
    public function onReceiveLogined(Context $context, Config $config)
    {
        ...
        if ($is_settle == 0) { //未结算
            switch ($StaffGrade) {
                case 1:
                    ...
                    break;
                case 2:
                    $sql = "SELECT minor_id as staff_id,minor_name as staff_name,major_id,major_name FROM staff_struct_minor WHERE minor_id = :staff_id";
                    //计算投注额
                    $sqls = "SELECT sum(wager_amount) as bet_all FROM monthly_staff WHERE minor_id = :staff_id AND monthly = :monthly";
                    break;
                case 3:
                    $sql = "SELECT agent_id as staff_id,agent_name as staff_name,minor_id,minor_name,major_id,major_name FROM staff_struct_agent WHERE agent_id = :staff_id";
                    //计算投注额
        ...
        if ($is_settle == 1) {  //已结算
            if ($StaffGrade == 1) {
                $sql = "SELECT major_name as staff_name,major_id as staff_id,bet_amount,bet_rate,profit_amount,profit_rate,fee_rate,tax_rate,dividend_result,deliver_time,settle_time FROM dividend_settle_major WHERE major_id = :major_id";
                $param = [":major_id" => $staffId];
            }
            if ($StaffGrade == 2) {
                $sql = "SELECT major_name,minor_name as staff_name, minor_id as staff_id bet_amount,bet_rate,profit_amount,profit_rate,fee_rate,tax_rate,dividend_result,deliver_time,settle_time FROM dividend_settle_minor WHERE minor_id = :minor_id";
                $param = [":minor_id" => $staffId];
            }
            if ($StaffGrade == 3) {
                $sql = "SELECT major_name,minor_name,agent_name as staff_name,agent_id as staff_id,bet_amount,bet_rate,profit_amount,profit_rate,fee_rate,tax_rate,dividend_result,deliver_time,settle_time FROM dividend_settle_agent WHERE agent_id = :agent_id";
            ...
            foreach ($lists as $key => $val) {
                ...
                $list[$key]["bet_rate"] = floatval($val["bet_rate"]);
                $list[$key]["profit_amount"] = $this->intercept_num($val["profit_amount"]);
                $list[$key]["profit_rate"] = floatval($val["profit_rate"]);
                $list[$key]["fee_rate"] = floatval($val["fee_rate"]);
                $list[$key]["tax_rate"] = floatval($val["tax_rate"]);
