 ...
 * @author： leo
 * @date：   2019-04-08   
 * @link：   System/SystemSetting/Shareholder
        ...
        if (!empty($major_list)) {
            foreach ($major_list as $key => $val) {
                ...
                if (!empty($dividend)) {
                    ...
                    $minors_list[$key]["self_bet"] = ($dividend["grade2_bet_rate"] != 0 ? floatval($dividend["grade2_bet_rate"]) : floatval($info['grade2_bet_rate']));
                    $minors_list[$key]["self_profit"] = ($dividend["grade2_profit_rate"] != 0 ? floatval($dividend["grade2_profit_rate"]) : floatval($info['grade2_profit_rate']));
                    $minors_list[$key]["self_fee"] = ($dividend["grade2_fee_rate"] != 0 ? floatval($dividend["grade2_fee_rate"]) : floatval($info['grade2_fee_rate']));
                    $minors_list[$key]["self_tax"] = ($dividend["grade2_tax_rate"] != 0 ? floatval($dividend["grade2_tax_rate"]) : floatval($info['grade2_tax_rate']));
                    $minors_list[$key]["agent_bet"] = ($dividend["grade3_bet_rate"] != 0 ? floatval($dividend["grade3_bet_rate"]) : floatval($info['grade3_bet_rate']));
                    $minors_list[$key]["agent_profit"] = ($dividend["grade3_profit_rate"] != 0 ? floatval($dividend["grade3_profit_rate"]) : floatval($info['grade3_profit_rate']));
                    $minors_list[$key]["agent_fee"] = ($dividend["grade3_fee_rate"] != 0 ? floatval($dividend["grade3_fee_rate"]) : floatval($info['grade3_fee_rate']));
                    $minors_list[$key]["agent_tax"] = ($dividend["grade3_tax_rate"] != 0 ? floatval($dividend["grade3_tax_rate"]) : floatval($info['grade3_tax_rate']));
                }
            }
        }
        sort($minors_list, 1);
