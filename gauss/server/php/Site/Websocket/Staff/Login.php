 ...
 * @author： leo
 * @date：   2019-04-08   
 * @link：   Staff/Login {"staff_key":"owner","staff_password":"123456"}
...
class Login implements IHandler
{
    public function onReceive(Context $context, Config $config)
    {
        ...
        // 为空判断
        if (empty($staff_key)) {
            $context->reply(['status' => 201, 'msg' => '请输入登陆账号']);
            return;
        }
        if (empty($password)) {
        ...
        if (empty($adminAuth)) {
            ...
            //查看当前用户是否绑定ip地址
            $bind_ip_sql = "SELECT bind_ip FROM staff_bind_ip WHERE staff_id = :staff_id";
            $bind_ip_info = [];
            foreach ($mysql->query($bind_ip_sql, $param) as $rows) {
                $bind_ip_info = $rows;
            }
            if (!empty($bind_ip_info)) {
                //判断当前登录的ip与绑定的ip是否匹配
                if (ip2long($context->getClientAddr()) != $bind_ip_info["bind_ip"]) {
                    $context->reply(["status" => 300, "msg" => "当前账户登录的ip地址错误"]);
                    return;
                }
            }
            //删除之前的登录信息
