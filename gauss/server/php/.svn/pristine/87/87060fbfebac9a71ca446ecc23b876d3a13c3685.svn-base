 ...
 * @Author  Rose
 * @date  2019-05-07
 * @links  Cash/WithdrawSetting/HandfeeSave {"list":[{"setting_key":"withdraw_free","value":6},{"setting_key":"withdraw_max","value":6000},{"setting_key":"withdraw_min","value":100}]}
 * @modifyAuthor   Rose
...
class HandfeeSave extends CheckLogin
{
    public function onReceiveLogined(Context $context, Config $config)
    {
        ...
        foreach ($list as $item) {
            $setting_key = $item['setting_key'];
            $value = $item['value'];
            if ($setting_key == 'withdraw_interval') {
                $sql = 'UPDATE site_setting SET int_value=:dbl_value WHERE setting_key=:setting_key';
            } else {
