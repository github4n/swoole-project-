...
/*
 * 我的--代理中心--下级报表--三级
 * User/Agent/TertiaryReport {"date":"today"}
 * date:yesterday;today,thisWeek;lastWeek;thisMonth;lastMonth
 * */

class TertiaryReport extends CheckLogin
{
    public function onReceiveLogined(Context $context, Config $config)
    {
        ...
        $sql = "SELECT user_id, user_key, deal_key, layer_id FROM user_info_intact WHERE broker_3_id = :broker_3_id";
        $param = [':broker_3_id'=> $userId];
        $primaryList = array();
        ...
        switch ($date)
        {
            case 'today':
                ...
                //今日新增人数
                $sql = 'SELECT COUNT(DISTINCT user_id) AS todayAdd FROM user_info WHERE broker_3_id=:broker_3_id AND register_time BETWEEN :today_start AND :today_stop';
                $params = [
                    ':broker_3_id' => $userId,
                    ':today_start' => $today_start,
                    ':today_stop' => $today_stop
                ...
                //今日下级总人数
                $sql = 'SELECT COUNT(DISTINCT user_id) AS todayAll FROM user_info WHERE broker_3_id=:broker_3_id AND register_time<=:today_stop';
                $params = [
                    'broker_3_id' => $userId,
                    ':today_stop' => $today_stop
                ];
                ...
                //今日下级人数
                $sql = 'SELECT COUNT(DISTINCT user_id) AS todayNum FROM user_info WHERE login_time BETWEEN :yesterday_start AND :yesterday_stop AND broker_3_id=:broker_3_id';
                $params = [
                    ':yesterday_start' => $today_start,
                    ':yesterday_stop' => $today_stop,
                    ':broker_3_id' => $userId
                ];
                foreach ($mysql->query($sql,$params) as $row)
                {
                ...
                //一级下线的当天佣金
                //提成比例
                $rate_sql = 'SELECT broker_3_rate FROM daily_user_brokerage WHERE daily=:daily AND user_id=:user_id';
                $param = [
                ...
                foreach ($primaryList as $k => $v)
                {
                    ...
                    {
                        $reportList[$k]['bet_all'] = empty($bet['bet_all'])?0:$bet['bet_all'];
                        $reportList[$k]['time'] = empty($bet['daily'])?date('Y-m-d',time()):date('Y-m-d',strtotime($bet['daily']));
                    }
            ...
            case 'thisWeek':
                ...
                $sql = 'SELECT COUNT(DISTINCT user_id) AS thisWeekAdd FROM user_info WHERE broker_3_id=:broker_3_id AND register_time BETWEEN :thisWeek_start AND :thisWeek_stop';
                $params = [
                    ':broker_3_id' => $userId,
                    ':thisWeek_start' => $thisWeek_start,
                    ':thisWeek_stop' => $thisWeek_stop
                ...
                //本周下级总人数
                $sql = 'SELECT COUNT(DISTINCT user_id) AS thisWeekAll FROM user_info WHERE broker_3_id=:broker_3_id AND register_time<=:thisWeek_stop';
                $params = [
                    'broker_3_id' => $userId,
                    ':thisWeek_stop' => $thisWeek_stop
                ];
                ...
                //本周下级人数
                $sql = 'SELECT COUNT(DISTINCT user_id) AS thisWeekNum FROM user_info WHERE login_time BETWEEN :thisWeek_start AND :thisWeek_stop AND broker_3_id=:broker_3_id';
                $params = [
                    ':thisWeek_start' => $thisWeek_start,
                    ':thisWeek_stop' => $thisWeek_stop,
                    ':broker_3_id' => $userId
                ...
                {
                    $broker_rate = $rate['broker_3_rate'];
                }
                $broker_rate = isset($broker_rate)?$broker_rate:0;
                ...
                foreach ($primaryList as $k => $v)
                {
                    ...
                    foreach ($reportMysql->query($bet_sql, $bet_param) as $bet)
                    {
                        ...
                        $reportList[$k]['time'] = empty($bet['weekly']) ? date('Y-m-d', strtotime('-1 Week')) : date('Y-m-d', strtotime($bet['weekly']));
                    }
            ...
            case 'lastWeek':
                ...
                $sql = 'SELECT COUNT(DISTINCT user_id) AS lastWeekAdd FROM user_info WHERE broker_3_id=:broker_3_id AND register_time BETWEEN :lastWeek_start AND :lastWeek_stop';
                //$context->reply($sql);
                $params = [
                    ':broker_3_id' => $userId,
                    ':lastWeek_start' => $lastWeek_start,
                ...
                //上周下级总人数
                $sql = 'SELECT COUNT(DISTINCT user_id) AS lastWeekAll FROM user_info WHERE broker_3_id=:broker_3_id AND register_time<=:lastWeek_stop';
                $params = [
                    'broker_3_id' => $userId,
                    ':lastWeek_stop' => $lastWeek_stop
                ];
                ...
                //上周下级人数
                $sql = 'SELECT COUNT(DISTINCT user_id) AS lastWeekNum FROM user_info WHERE login_time BETWEEN :lastWeek_start AND :lastWeek_stop AND broker_3_id=:broker_3_id';
                $params = [
                    ':lastWeek_start' => $lastWeek_start,
                    ':lastWeek_stop' => $lastWeek_stop,
                    ':broker_3_id' => $userId
            ...
            case 'thisMonth':
                ...
                $sql = 'SELECT COUNT(DISTINCT user_id) AS thisMonthAdd FROM user_info WHERE broker_3_id=:broker_3_id AND register_time BETWEEN :thisMonth_start AND :thisMonth_stop';
                $params = [
                    ':broker_3_id' => $userId,
                    ':thisMonth_start' => $thisMonth_start,
                    ':thisMonth_stop' => $thisMonth_stop
                ...
                //本月下级总人数
                $sql = 'SELECT COUNT(DISTINCT user_id) AS thisMonthAll FROM user_info WHERE broker_3_id=:broker_3_id AND register_time<=:thisMonth_stop';
                $params = [
                    'broker_3_id' => $userId,
                    ':thisMonth_stop' => $thisMonth_stop
                ];
                ...
                //本月下级人数
                $sql = 'SELECT COUNT(DISTINCT user_id) AS thisMonthNum FROM user_info WHERE login_time BETWEEN :thisMonth_start AND :thisMonth_stop AND broker_3_id=:broker_3_id';
                $params = [
                    ':thisMonth_start' => $thisMonth_start,
                    ':thisMonth_stop' => $thisMonth_stop,
                    ':broker_3_id' => $userId
            ...
            case 'lastMonth':
                ...
                $sql = 'SELECT COUNT(DISTINCT user_id) AS lastMonthAdd FROM user_info WHERE broker_3_id=:broker_3_id AND register_time BETWEEN :lastMonth_start AND :lastMonth_stop';
                $params = [
                    ':broker_3_id' => $userId,
                    ':lastMonth_start' => $lastMonth_start,
                    ':lastMonth_stop' => $lastMonth_stop
                ...
                //上月下级总人数
                $sql = 'SELECT COUNT(DISTINCT user_id) AS lastMonthAll FROM user_info WHERE broker_3_id=:broker_3_id AND register_time<=:lastMonth_stop';
                $params = [
                    'broker_3_id' => $userId,
                    ':lastMonth_stop' => $lastMonth_stop
                ];
                ...
                //上月下级人数
                $sql = 'SELECT COUNT(DISTINCT user_id) AS lastMonthNum FROM user_info WHERE login_time BETWEEN :lastMonth_start AND :lastMonth_stop AND broker_3_id=:broker_3_id';
                $params = [
                    ':lastMonth_start' => $lastMonth_start,
                    ':lastMonth_stop' => $lastMonth_stop,
                    ':broker_3_id' => $userId
            ...
            default:
                ...
                //昨日新增人数
                $sql = 'SELECT COUNT(DISTINCT user_id) AS yesterdayAdd FROM user_info WHERE broker_3_id=:broker_3_id AND register_time BETWEEN :yesterday_start AND :yesterday_stop';
                $params = [
                    ':broker_3_id' => $userId,
                    ':yesterday_start' => $yesterday_start,
                    ':yesterday_stop' => $yesterday_stop
                ...
                //昨日下级总人数
                $sql = 'SELECT COUNT(DISTINCT user_id) AS yesterdayAll FROM user_info WHERE broker_3_id=:broker_3_id AND register_time<=:yesterday_stop';
                $params = [
                    'broker_3_id' => $userId,
                    ':yesterday_stop' => $yesterday_stop
                ];
                ...
                //昨日下级人数
                $sql = 'SELECT COUNT(DISTINCT user_id) AS yesterdayNum FROM user_info WHERE login_time BETWEEN :yesterday_start AND :yesterday_stop AND broker_3_id=:broker_3_id';
                $params = [
                    ':yesterday_start' => $yesterday_start,
                    ':yesterday_stop' => $yesterday_stop,
                    ':broker_3_id' => $userId
