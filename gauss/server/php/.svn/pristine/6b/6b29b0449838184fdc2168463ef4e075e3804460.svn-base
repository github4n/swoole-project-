...
/*
 * 我的--代理中心--下级列表--二级代理
 * User/Agent/SecondaryAgent
 * */

class SecondaryAgent extends CheckLogin
{
    public function onReceiveLogined(Context $context, Config $config)
    {
        ...
        $num = $data["num"]?intval($data["num"]):15;
        $limit = " LIMIT ".($page-1)*$num.",".$num;
        $mysql = $config->data_user;
        $sql = "SELECT user_key,register_time,login_time FROM user_info_intact WHERE broker_2_id=:broker_2_id".$limit;
        $total_sql = "SELECT user_id FROM user_info_intact WHERE broker_2_id=:broker_2_id";
        $lists = array();
        $param = [":broker_2_id"=>$context->getInfo("UserId")];
        $total = 0;
        try{
