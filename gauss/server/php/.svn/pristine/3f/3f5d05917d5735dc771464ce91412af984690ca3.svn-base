...
/*
 * Number.php
 * @description   开奖号码推送app任务
 * @Author  rose 
 * @date  2019-05-09
 * @links  Lottery/Number 
 * @modifyAuthor   nathan
...
class Number implements IHandler
{
    public function onTask(Context $context, Config $config)
    {
        ...
        $period_sql = 'select period from lottery_period where game_key=:game_key and period = :period';
        $true_period = '';
        foreach ($mysql->query($period_sql, [':game_key' => $game_key, ':period' => $period]) as $val) {
            $true_period = $val['period'];
        }
        if (!$true_period) {
            return;
        }
        $sql = 'select * from lottery_number where game_key=:game_key and period=:period';
        $params = ['game_key' => $game_key, 'period' => $true_period];
