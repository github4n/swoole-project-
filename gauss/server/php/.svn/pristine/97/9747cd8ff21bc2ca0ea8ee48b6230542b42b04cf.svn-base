 ...
 * BrokerageThree.php.
 *
 * @description   代理返佣--三级下线详情
 * @Author  Luis
 * @date  2019-04-07
 * @links  参数：AgentRebate/Brokerage/BrokerageThree {"broker_id":1,"time":"20190124"}
 * @modifyAuthor   Luis
...
class BrokerageThree extends CheckLogin
{
    public function onReceiveLogined(Context $context, Config $config)
    {
        ...
        $sql = 'select user_key,wager_amount from daily_user where broker_3_id = :broker_3_id and daily=:daily and bet_amount >0';
        ...
        if (!empty($userDate) && !empty($brokerDate)) {
            foreach ($userDate as $k => $v) {
                ...
                $list[$k]['brokerage'] = $this->intercept_num($list[$k]['bet_amount'] * $list[$k]['broker_3_rate'] * 0.01);
            }
        }
