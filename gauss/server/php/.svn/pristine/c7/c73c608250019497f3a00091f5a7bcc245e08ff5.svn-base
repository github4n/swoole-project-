        ...
        if (!empty($major_list)) {
            foreach ($major_list as $key => $val) {
                ...
                if (!empty($dividend)) {
                    ...
                    $majors_list[$key]["self_bet"] = ($dividend["grade3_bet_rate"] != 0 ? floatval($dividend["grade3_bet_rate"]) : floatval($info['grade3_bet_rate']));
                    $majors_list[$key]["self_profit"] = ($dividend["grade3_profit_rate"] != 0 ? floatval($dividend["grade3_profit_rate"]) : floatval($info['grade3_profit_rate']));
                    $majors_list[$key]["self_fee"] = ($dividend["grade3_fee_rate"] != 0 ? floatval($dividend["grade3_fee_rate"]) : floatval($info['grade3_fee_rate']));
                    $majors_list[$key]["self_tax"] = ($dividend["grade3_tax_rate"] != 0 ? floatval($dividend["grade3_tax_rate"]) : floatval($info['grade3_tax_rate']));
                }
            }
        }
        sort($majors_list, 1);
