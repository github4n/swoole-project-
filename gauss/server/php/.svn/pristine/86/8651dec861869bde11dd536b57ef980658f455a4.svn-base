 ...
 * @Author  Rose
 * @date  2019-05-07
 * @links  Cash/Simple/SimpleDeposit {"user_id":1,"money":200,"route_id":3,"memo":""}
 * @modifyAuthor   Rose
    ...
    public function onReceiveLogined(Context $context, Config $config)
    {
        ...
        if (!is_numeric($money)) {
            ...
            if (!preg_match('/^[0-9]+(.[0-9]{1,2})?$/', $money)) {
                $money = $this->intercept_num($money);
            }
        }

        //获取便捷支付的基本信息
        $sql = 'select passage_id,passage_name,pay_url,min_money,max_money,passage_acceptable,acceptable,risk_control,cumulate from deposit_route_simple_intact where route_id = :route_id';
        $route_info = [];
        ...
        if ($route_info['passage_acceptable'] != 1 || $route_info['acceptable'] != 1) {
            $context->reply(['status' => 209, 'msg' => '通道停用,请选择正确的开启状态的通道']);

            return;
        }
        if ($route_info['cumulate'] >= $route_info['risk_control']) {
            $context->reply(['status' => 209, 'msg' => '该通道超出风控金额']);
        ...
        $sql = 'insert into deposit_launch set user_id=:user_id,user_key=:user_key,account_name=:account_name,layer_id=:layer_id,passage_id=:passage_id,passage_name=:passage_name,launch_money=:launch_money,launch_device=:launch_device,route_id=:route_id';
        $param = [
            ':user_id' => $user_id,
        ...
        //入款完成
        $sql = 'insert into deposit_finish set deposit_serial=:deposit_serial,finish_money=:finish_money,coupon_audit=:coupon_audit,deposit_audit=:deposit_audit,finish_time=:finish_time,finish_staff_id=:finish_staff_id,finish_staff_name=:finish_staff_name';
        $params = [
            ':deposit_serial' => $deposit_serial,
            ':finish_money' => $money,
            ':coupon_audit' => 0,
