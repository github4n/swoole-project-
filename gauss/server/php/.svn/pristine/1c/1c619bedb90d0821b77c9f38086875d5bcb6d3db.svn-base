 ...
 * @links  Roles/RolesUpdate  {"role_name":"测试管理员","role_auth":["account_admin_delete","account_admin_insert","account_admin_select","account_admin_update","account_operate_select","account_role_delete","account_role_insert","account_role_select","account_role_update","cash_list","report_analysis","report_monthly","report_site","ticket_official_select","ticket_official_update","ticket_play_select","ticket_play_update","ticket_result_select","ticket_self_select","ticket_self_update","ticket_setting_bet","ticket_setting_bonus","ticket_setting_rebate","ticket_setting_select"]}
 * @modifyAuthor   avery
...
class RolesUpdate extends CheckLogin
{
    public function onReceiveLogined(Context $context, Config $config)
    {
        ...
        if (mb_strlen($role_name) > 10 || mb_strlen($role_name) < 2) {
            $context->reply(['status' => 210, 'msg' => '请输入角色名称,2-10位']);
        ...
        foreach ($role_auth as $item) {
            ...
            if ($operate_data == null) {
                $context->reply(['status' => 401, 'msg' => '权限分配失败']);

                return;
            }
            try {
                $sqls = 'INSERT INTO admin_permit SET role_id=:role_id,operate_key=:operate_key ';
                $params = [
                    ':role_id' => $role_id,
                    ':operate_key' => $item,
                ];
                ...
                $context->reply(['status' => 402, 'msg' => '权限分配失败']);
                throw new \PDOException($e);
            }
        }
        //添加日志
