...
class LayerMemberAuto implements IHandler
{
    public function onTask(Context $context, Config $config)
    {
        try {
                ...
                foreach ($user_list as $k => $v) {
                    ...
                    if ($layer_type == 103) {
                        ...
                        foreach ($mysqlUser->query($sql, [':user_id' => $v['user_id']]) as $layerNew) {
                            $start_time = intval($layerNew['start_time']);
                        }

                        if (floor((strtotime('today') - $start_time) / 86400) >= $max_day) {
                            //新代理的数据
                            $data_sql = 'select sum(deposit_amount) as deposit_amount,sum(wager_amount) as wager_amount,count(distinct (if(wager_amount>0,user_id,null))) as user_count from daily_user where (broker_1_id = :user_id or broker_2_id = :user_id or broker_3_id = :user_id ) and daily <= :daily';
                            ...
                            $userData = [];
                            foreach ($mysqlReport->query($data_sql, [':user_id' => $v['user_id'], ':daily' => $daily]) as $user) {
                                $userData = $user;
                            }

                            $layer_lists = iterator_to_array($mysqlUser->query($sql));
                            if (!empty($layer_lists)) {
                                foreach ($layer_lists as $key => $val) {
            ...
            $adapter->plan('Report/UserDeal', ['time' => time() + 600], time() + 600, 9);
        }
    }
}
