...
class ApiLottery extends Base
{
    private const URLFORMAT = 'https://spider.xblan.cc/apilottery/%s.json';
    ...
    {
        $url = sprintf(self::URLFORMAT, 'xylc');
        $json = $this->httpGet($url);
        $data = json_decode($json, true);
        if (empty($data['data'])) {
            return;
    ...
    {
        $url = sprintf(self::URLFORMAT, 'gdkl10f');
        $json = $this->httpGet($url);
        $data = json_decode($json, true);
        if (empty($data['data'])) {
            return;
    ...
    {
        $url = sprintf(self::URLFORMAT, 'gd11x5');
        $json = $this->httpGet($url);
        $data = json_decode($json, true);
        if (empty($data['data'])) {
            return;
    ...
    {
        $url = sprintf(self::URLFORMAT, 'xj6hc');
        $json = $this->httpGet($url);
        $data = json_decode($json, true);
        if (empty($data['data'])) {
            return;
