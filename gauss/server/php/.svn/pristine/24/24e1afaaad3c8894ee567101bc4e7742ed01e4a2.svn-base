...
class OpenCai extends Base
{
    private const URLFORMAT = 'https://spider.xblan.cc/opencai/%s.json';
    ...
    {
        $url = sprintf(self::URLFORMAT, 'mlaft');
        $json = $this->httpGet($url);
        $data = json_decode($json, true);
        if (empty($data['data'])) {
            return;
