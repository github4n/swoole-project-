...
/*
 * 我的--代理中心--下级列表--三级代理
 * User/Agent/TertiaryAgent
 * */

class TertiaryAgent extends CheckLogin
{
    public function onReceiveLogined(Context $context, Config $config)
    {
        ...
        $sql = "SELECT user_key,register_time,login_time FROM user_info_intact WHERE broker_3_id=:broker_3_id".$limit;
        $total_sql = "SELECT user_id FROM user_info_intact WHERE broker_3_id=:broker_3_id";
        $lists = array();
        $total = 0;
        $param = [":broker_3_id"=>$context->getInfo("UserId")];
        try{
