...
create table layer_message(
    ...
    insert_time int unsigned not null comment '创建时间',
...
create table layer_info(
    ...
    min_deposit_amount decimal(14,2) not null default 0 comment '条件-充值金额',
    min_bet_amount decimal(14,2) not null default 0 comment '条件-有效投注金额',
    min_deposit_user int not null default 0 comment '条件-首充人数',
    max_day int not null default 0 comment '条件-天数',
    withdraw_audit_amount decimal(14,2) not null default 10000 comment '出款审核下限',
...
create table coupon_daily_setting(
    layer_id int unsigned not null comment '层级id',
    bet_money_1 decimal(14,2) not null comment '投注额1挡',
    coupon_rate_1 decimal(4,2) not null comment '加奖比例1挡',
    bet_money_2 decimal(14,2) not null comment '投注额2挡',
    coupon_rate_2 decimal(4,2) not null comment '加奖比例2挡',
    bet_money_3 decimal(14,2) not null comment '投注额3挡',
    coupon_rate_3 decimal(4,2) not null comment '加奖比例3挡',
    audit_rate decimal(4,2) not null comment '活动稽核倍数',
    primary key(layer_id)
) comment '每日加奖设置';;
...
create table coupon_upgrade_setting(
    ...
    coupon_money decimal(14,2) not null comment '晋级彩金',
    audit_rate decimal(4,2) not null comment '活动稽核倍数',
    primary key(layer_id)
) comment '晋级奖励设置';;
...
create table coupon_deposit_setting(
    deposit_money decimal(14,2) not null comment '充值金额',
    coupon_money decimal(14,2) not null comment '送彩金',
    audit_rate decimal(4,2) not null comment '活动稽核倍数',
...
create table subsidy_game_setting(
    ...
    min_bet decimal(14,2) not null comment '打码量',
    subsidy_rate decimal(4,2) not null comment '反水比例%',
    max_subsidy decimal(14,2) not null comment '反水上限',
...
create table bank_info(
    ...
    withdraw_amount decimal(14,2) not null comment '累计出款金额',
...
create table brokerage_setting(
    ...
    min_bet_amount decimal(14,2) not null comment '活跃会员最低投注额',
    min_deposit decimal(14,2) not null comment '活跃会员最低充值金额',
    auto_deliver tinyint not null default 0 comment '自动派发',
    deliver_time int not null default 0 comment '派发时间',
    primary key(layer_id)
) comment '代理佣金设置';;
...
create table brokerage_rate(
    ...
    broker_1_rate decimal(4,2) not null comment '一级下线佣金比例',
    broker_2_rate decimal(4,2) not null comment '二级下线佣金比例',
    broker_3_rate decimal(4,2) not null comment '三级下线佣金比例',
...
begin
    set new.insert_time=unix_timestamp();
end;;
