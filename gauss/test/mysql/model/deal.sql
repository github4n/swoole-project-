...
create table external_import_launch(
    ...
    external_key varchar(100) not null comment '三方平台：唯一性检测',
    launch_data json not null comment '外接口数据',
    launch_money decimal(14,2) not null comment '转入金额',
    launch_time int unsigned not null comment '提交时间',
    unique(external_type,external_key),
...
create table staff_deposit(
    ...
    account_name varchar(20) not null comment '真实姓名',
    layer_id int unsigned not null comment '代理/会员层级id',
    money decimal(14,2) not null comment '入款金额',
...
create table external_audit(
	...
	audit_amount decimal(14,2) not null comment '打码金额',
...
create table external_export_launch(
    ...
    external_key varchar(100) not null comment '三方平台：唯一性检测',
    launch_data json not null comment '外接口数据',
    launch_money decimal(14,2) not null comment '转出金额',
...
create table withdraw_launch(
    ...
    user_key varchar(20) not null comment '登陆名',
    layer_id int unsigned not null comment '代理/会员层级id',
    launch_money decimal(14,2) not null comment '提款金额',
    deposit_audit decimal(14,2) not null comment '稽核金额',
    handling_fee decimal(14,2) not null comment '手续费',
    withdraw_money decimal(14,2) not null comment '出款金额',
...
create table deposit_launch(
    ...
    launch_money decimal(14,2) not null comment '提交金额',
    launch_time int unsigned not null comment '提交时间',
    launch_device tinyint unsigned not null comment '来源设备类型：0-PC，1-手机浏览器，2-苹果手机app，3-安卓手机app',
...
create table deposit_gateway(
    ...
    to_account_number varchar(100) not null comment '商户号',
    to_account_name varchar(100) not null comment '商户名称',
    primary key(deposit_serial)
) comment '三方入款';;
...
create table deposit_finish(
    ...
    finish_money decimal(14,2) not null comment '实际存入金额',
    deposit_audit decimal(14,2) not null comment '充值稽核',
    coupon_audit decimal(14,2) not null comment '优惠稽核',
...
create table staff_withdraw(
    ...
    account_name varchar(20) not null comment '真实姓名',
    layer_id int unsigned not null comment '代理/会员层级id',
    money decimal(14,2) not null comment '出款金额',
    deposit_audit decimal(14,2) not null comment '入款稽核',
    coupon_audit decimal(14,2) not null comment '活动稽核',
    memo varchar(100) not null comment '备注',
...
create table brokerage_deliver(
    ...
    layer_id int unsigned not null comment '代理/会员层级id',
    daily int unsigned not null comment '天 php:intval(date(Ymd))',
    brokerage decimal(14,2) not null comment '佣金',
    deliver_time int unsigned not null comment '派发时间',
    unique(user_id,daily),
    primary key(deliver_deal_serial)
) comment '佣金结算';;
